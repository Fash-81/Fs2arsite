<template>
    <NavBar/>
    <h1 class="text-5xl mt-10 text-center  text-white font-Negar">پوستر ها</h1>
      <div class="overflow-x-auto items-center sm:rounded-lg w-7/8 mx-20 mb-10 ">
        <table class=" w-full text-xl text-center  font-Negar  -white mt-20">
            ...
            <tbody v-for="Portfilio in getPosters"
                    :key="Portfilio">
...
export default {
    ...
    computed: {
        getPosters() {
            return this.nemuoneKareman.filter(p => p.case === 'پوستر')
        }
    },
    mounted() {
